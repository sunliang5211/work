...
def get_model(layers = [20,10], reg_layers=[0,0]):
    ...
    # Input variables
    user_input = Input(shape=(35,), dtype='float32', name = 'user_input')
    item_input = Input(shape=(55,), dtype='float32', name = 'item_input')

    vector = merge([user_input, item_input], mode = 'concat')
    
    # MLP layers
    for idx in xrange(1, num_layer):
        layer = Dense(layers[idx], init = init_normal ,W_regularizer= l2(reg_layers[idx]), activation='relu', name = 'layer%d' %idx)
        vector = layer(vector)
